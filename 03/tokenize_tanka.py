...
for row in csv.reader(iter(sys.stdin.readline, '')):
    ...
    for line in lines:
        ...
        # 空行を読み飛ばす
        if len(token_and_info) is not 3:
            continue

        # 品詞と原形を抽出する
        [token, word_class, basic_form] = token_and_info
        ...
        # 形態素・品詞・原形を保存
        tokens.append([token, word_class, basic_form])

    # 形態素列をJSONに変換
    tokens_json = json.dumps(tokens)

    # CSVとして書き出し
    csv_writer.writerow([tanka, tokens])
